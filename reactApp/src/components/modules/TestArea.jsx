...
// // // /**
// // //  * TestArea
// // //  * - Fetches questions from GET /api/questions/:id
// // //  * - Renders questions + options dynamically
// // //  * - Lets user select options
// // //  * - Computes score by comparing to question.correctAnswer
// // //  *
// // //  * Expected question shape from API:
// // //  * {
// // //  *   _id,
// // //  *   testId,
// // //  *   questionText,
...
// // //   const base = "flex items-center p-3 my-2 border rounded-xl cursor-pointer transition duration-150 shadow-sm";
// // //   const sel = "border-emerald-500 ring-2 ring-emerald-300 bg-emerald-50";
// // //   const unsel = "border-gray-200 hover:border-gray-400 bg-white";

// // //   return (
// // //     <div className={`${base} ${isSelected ? sel : unsel}`} onClick={onSelect}>
// // //       <div className={`w-8 h-8 flex items-center justify-center font-bold text-sm rounded-full mr-4 ${isSelected ? "bg-emerald-500 text-white" : "bg-gray-200 text-gray-700"}`}>
// // //         {label}
// // //       </div>
// // //       <div className="text-gray-900 text-md">{value}</div>
// // //     </div>
// // //   );
// // // };

// // // const TestArea = () => {
// // //   const { id } = useParams(); // this is testId from card
// // //   const [questions, setQuestions] = useState([]);
// // //   const [selected, setSelected] = useState({}); // { questionIndex: "A" }
...
// // //       } catch (err) {
// // //         console.error(err);
// // //         if (mountedRef.current) setError("Could not load questions. Check server.");
// // //       } finally {
...
// // //       // q.correctAnswer expected "A"/"B"/ etc
// // //       if (!q.correctAnswer) return;
// // //       const userAns = selected[i];
// // //       if (userAns && userAns.toUpperCase() === q.correctAnswer.toUpperCase()) correct++;
// // //     });
// // //     setScore({ correct, total: questions.length, percent: questions.length ? Math.round((correct / questions.length) * 100) : 0 });

// // //     // optional: send results to server (commented — enable if you have endpoint)
// // //     // fetch('/api/results', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ testId: id, score: correct, total: questions.length, answers: selected }) })
// // //     //  .then(...)
// // //   };
...
// // //   if (loading) return <div className="min-h-screen flex items-center justify-center p-6">Loading questions...</div>;
// // //   if (error) return <div className="min-h-screen flex items-center justify-center p-6 text-red-500">{error}</div>;
...
// // //         <div className="max-w-4xl mx-auto">
// // //           {/* Timer placeholder */}
// // //           <div className="bg-amber-100 border border-amber-300 text-amber-800 p-3 rounded-xl mb-6 shadow-md flex items-center justify-between">
// // //             <span className="font-bold tracking-wider">TIME LEFT - 09:15 MIN (Test ID: {id})</span>
// // //             {score && <span className="text-sm font-semibold">Score: {score.correct}/{score.total} ({score.percent}%)</span>}
// // //           </div>

// // //           {/* Questions */}
// // //           {questions.length === 0 && <p className="text-center text-gray-500">No questions found for this test.</p>}

// // //           {questions.map((q, i) => (
// // //             <div key={q._id || i} className="bg-white p-6 rounded-xl shadow-lg mb-8">
// // //               <p className="text-gray-700 mb-4 text-lg">{i + 1}. {q.questionText}</p>

// // //               {q.options && q.options.map((opt, j) => {
// // //                 const label = labels[j] || `opt${j}`;
...
// // //             <button onClick={handleSubmit} className="px-6 py-3 bg-emerald-500 text-white font-semibold rounded-xl shadow hover:bg-emerald-600">
// // //               Submit Test
// // //             </button>

// // //             <button onClick={() => { setSelected({}); setScore(null); }} className="px-6 py-3 bg-gray-100 text-gray-800 rounded-xl shadow hover:bg-gray-200">
// // //               Reset Answers
// // //             </button>
// // //           </div>
// // //         </div>
...
// // //           {questions.map((_, i) => {
// // //             const state = selected[i] ? "answered" : "unanswered";
// // //             return (
// // //               <button key={i} className={`w-full h-10 flex items-center justify-center rounded-lg font-bold shadow ${getButtonClass(state)}`}>
// // //                 {i + 1}
// // //               </button>
// // //             );
// // //           })}
...
// // //         <div className="mb-4">
// // //           <p className="text-sm text-gray-600">Answered: <span className="font-semibold">{Object.keys(selected).length}</span></p>
// // //           <p className="text-sm text-gray-600">Total: <span className="font-semibold">{questions.length}</span></p>
// // //           {score && <p className="text-sm text-gray-700 mt-2">Score: <span className="font-semibold">{score.correct}/{score.total} ({score.percent}%)</span></p>}
// // //         </div>

// // //         <button onClick={handleSubmit} className="w-full py-3 bg-emerald-500 text-white font-semibold rounded-xl shadow-lg hover:bg-emerald-600">
// // //           Submit Test
// // //         </button>
// // //       </div>
// // //     </div>
...
// //       if (ans && ans.toUpperCase() === q.correctAnswer.toUpperCase()) correct++;
// //     });
...
// //   if (!questions.length) return <div className="p-6 text-center">No questions found for this test.</div>;

// //   return (
// //     <div className="min-h-screen p-6 flex flex-col lg:flex-row gap-6 bg-gray-50">
// //       <div className="flex-1">
// //         {questions.map((q, i) => (
// //           <div key={q._id} className="bg-white p-6 rounded-xl shadow mb-6">
// //             <p className="mb-4 font-semibold text-gray-700">{i + 1}. {q.questionText}</p>
// //             {q.options.map((opt, j) => {
...
// //               );
// //             })}
// //             {score && <p className="mt-2 text-sm text-gray-600">Correct answer: {q.correctAnswer}</p>}
// //           </div>
// //         ))}

// //         <div className="flex gap-4">
// //           <button onClick={handleSubmit} className="px-6 py-3 bg-emerald-500 text-white rounded-xl shadow hover:bg-emerald-600">Submit Test</button>
// //           <button onClick={() => { setSelected({}); setScore(null); }} className="px-6 py-3 bg-gray-100 text-gray-800 rounded-xl shadow hover:bg-gray-200">Reset</button>
// //         </div>

// //         {score && <p className="mt-4 font-semibold">Score: {score.correct}/{score.total} ({score.percent}%)</p>}
// //       </div>
// //     </div>
// //   );
// // };
...
//         if (mountedRef.current) setQuestions(json.data || []);
//       } catch (err) {
//         console.error(err);
//         if (mountedRef.current) setError("Could not load questions. Check server.");
//       } finally {
...
//   if (loading) return <div className="min-h-screen flex items-center justify-center p-6">Loading questions...</div>;
//   if (error) return <div className="min-h-screen flex items-center justify-center p-6 text-red-500">{error}</div>;
//   if (!questions.length) return <div className="min-h-screen flex items-center justify-center p-6">No questions found.</div>;
  ...
  useEffect(() => {
    ...
    async function fetchQuestions() {
      try {
        ...
        console.error(err);
        setError("Could not load questions. Check server/API.");
      } finally {
        if (mountedRef.current) setLoading(false);
      }
    }
            ...
            <div key={q._id || i} className="bg-white p-6 rounded-xl shadow-lg mb-8">
              ...
              {q.options?.map((opt, j) => (
                <Option
                  ...
                  label={labels[j]}
                  value={opt}
                  isSelected={selected[i] === labels[j]}
                  onSelect={() => handleSelect(i, labels[j])}
                />
              ))}
      ...
      <div className="w-full lg:w-96 bg-white p-6 border-l border-gray-200 shadow-xl">
        ...
        <div className="grid grid-cols-5 gap-3 mb-4">
          {questions.map((_, i) => (
            <button
              key={i}
              className={`w-full h-10 flex items-center justify-center rounded-lg font-bold shadow ${
                selected[i] ? "bg-emerald-500 text-white hover:bg-emerald-600" : "bg-blue-500 text-white hover:bg-blue-600"
              }`}
            >
              {i + 1}
            </button>
